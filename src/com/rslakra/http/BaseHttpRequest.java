...
import com.rslakra.http.HttpUtils.KeyValuePair;
...
public abstract class BaseHttpRequest implements HttpRequest {
	 ...
	 * @return
	 */
	public static String getUserAgentString(final String mPlatform, final String mAppName, final String mAppBundleIdentifier, final String mServerVersion, final String mAppType) {
		StringBuilder userAgentBuilder = new StringBuilder();
		
		/* These properties are mandatory for the server requests. */
		if(HttpUtils.isNullOrEmpty(mPlatform) || HttpUtils.isNullOrEmpty(mAppName) || HttpUtils.isNullOrEmpty(mAppBundleIdentifier) || HttpUtils.isNullOrEmpty(mAppType)) {
	 ...
	 * @return
	 */
	public static String getUserAgentString(final String mPlatform, final String mAppName, final String mAppBundleIdentifier, final String mAppType) {
	...
	public static HttpURLConnection openHttpURLConnection(URL url, Proxy proxy) throws IOException {
		return (HttpUtils.isNotNull(url) ? (HttpURLConnection) (HttpUtils.isNotNull(proxy) ? url.openConnection(proxy) : url.openConnection()) : null);
	}
	...
	public static HttpsURLConnection openHttpsURLConnection(URL url, Proxy proxy) throws IOException {
		return (HttpUtils.isNotNull(url) && url.getProtocol().equals("https") ? (HttpsURLConnection) (HttpUtils.isNotNull(proxy) ? url.openConnection(proxy) : url.openConnection()) : null);
	}
	 ...
	 * @throws IOException
	 */
	public static void setConnectAndReadTimeouts(HttpURLConnection urlConnection, int timeoutInMillis, int readTimeoutInMillis) throws IOException {
		if(HttpUtils.isNotNull(urlConnection)) {
	...
	public static void setDefaultConnectAndReadTimeouts(HttpURLConnection urlConnection) throws IOException {
		setConnectAndReadTimeouts(urlConnection, Constants.HTTP_CONNECTION_TIMEOUT_SECONDS * 1000, Constants.HTTP_READ_TIMEOUT_SECONDS * 1000);
	}
	 ...
	 * @throws IOException
	 */
	public static void setDoInputAndDoOutput(final HttpURLConnection urlConnection, final boolean doInput, final boolean doOutput) throws IOException {
		if(HttpUtils.isNotNull(urlConnection)) {
			/*
	 ...
	 * @throws IOException
	 */
	public static void setUseCaches(final HttpURLConnection urlConnection, final boolean useCaches, final boolean defaultUseCaches) throws IOException {
		if(HttpUtils.isNotNull(urlConnection)) {
			/*
	 ...
	 * @throws IOException
	 */
	public static void setRequestMethod(final HttpURLConnection urlConnection, final String requestMethod) throws IOException {
		if(HttpUtils.isNotNull(urlConnection)) {
			/*
			 * Sets the flag indicating whether this URLConnection allows
	 ...
	 * @throws IOException
	 */
	public static void addHeader(final HttpURLConnection urlConnection, final String fieldName, final String fieldValue) throws IOException {
	 ...
	 * @throws IOException
	 */
	public static void setHeader(final HttpURLConnection urlConnection, final String fieldName, final String fieldValue) throws IOException {
		if(HttpUtils.isNotNull(urlConnection) && HttpUtils.isNotNullOrEmpty(fieldName)) {
	...
	public static void setQueryString(HttpURLConnection urlConnection, final String queryString) throws IOException {
		if(HttpUtils.isNotNull(urlConnection) && !isGetRequest(urlConnection) && HttpUtils.isNotNullOrEmpty(queryString)) {
	 ...
	 * @param requestParameters
	 */
	public static void addRequestHeadersFromParameters(HttpURLConnection urlConnection, Map<String, Object> requestParameters) {
		if(HttpUtils.isNotNull(urlConnection) && !HttpUtils.isNullOrEmpty(requestParameters)) {
			// add CONTENT_TYPE in headers, if available
			String contentType = HTTPUtil.getValueForKeyAsString(requestParameters, Headers.CONTENT_TYPE);
