...
public final class HTTPUtil {
	...
	public static <T> boolean contains(Collection<T> collection, T value) {
		return ((!IOUtil.isNullOrEmpty(collection)) && collection.contains(value));
	}
	...
	public static <K, V> boolean keyExists(Map<K, V> keyValues, K key) {
		return (!IOUtil.isNullOrEmpty(keyValues) && keyValues.keySet().contains(key));
	}
	...
	public static Set<String> asSet(String valueString, String delimiter) {
		...
		if(IOUtil.isNotNullOrEmpty(valueString) && IOUtil.isNotNullOrEmpty(delimiter)) {
			String[] tokens = valueString.split(delimiter);
			if(!IOUtil.isNullOrEmpty(tokens)) {
	...
	public static <T> boolean equals(T type, T... types) {
		...
		if(IOUtil.isNotNull(type) && !IOUtil.isNullOrEmpty(types)) {
	...
	public static boolean equalsAnyone(boolean ignoreCase, String string, String... args) {
		if(IOUtil.isNotNullOrEmpty(string) && args != null) {
			for(int i = 0; i < args.length; i++) {
				if(ignoreCase) {
	...
	public static boolean isInstanceOf(Object object, Class<?>... classTypes) {
		...
		if(IOUtil.isNotNull(object) && !IOUtil.isNullOrEmpty(classTypes)) {
	...
	public static String getClassSimpleName(Class<?> klass) {
		return (IOUtil.isNull(klass) ? null : klass.getSimpleName());
	}
	...
	public static String getClassSimpleName(Object object) {
		...
		if(IOUtil.isNotNull(object)) {
			Class<?> klass = object.getClass().getEnclosingClass();
			if(IOUtil.isNull(klass)) {
	...
	public static void setDeviceModel(String deviceModel) {
		HTTPUtil.deviceModel = deviceModel;
	}
	...
	public static String getUserAgentString(String appBundleIdentifier, String appType, String deviceModel) {
		...
		appBundleIdentifier = (IOUtil.isNullOrEmpty(appBundleIdentifier) ? APP_PACKAGE_NAME : appBundleIdentifier);
		appType = (IOUtil.isNullOrEmpty(appType) ? APP_TYPE : appType);
	...
	public static URL newURL(String baseUrl, String urlSuffix) throws IOException {
		URL url = null;
		if(IOUtil.isNullOrEmpty(urlSuffix)) {
			url = newURL(baseUrl);
		} else {
	...
	public static String getServerUrl(String baseServerUrl, String urlSuffix) {
		...
		if(IOUtil.isNotNullOrEmpty(baseServerUrl)) {
			urlString.append(baseServerUrl);
		}
		
		// append urlPrefix, if available.
		if(!IOUtil.isNullOrEmpty(urlSuffix)) {
			if(urlSuffix.startsWith(IOUtil.SLASH)) {
				urlString.append(urlSuffix);
			} else {
				urlString.append(IOUtil.SLASH).append(urlSuffix);
			}
		}
	...
	public static String getHostName(String urlString) {
		...
		if(IOUtil.isNullOrEmpty(hostName)) {
	...
	public static boolean isServerReachable(String baseServerUrl, String urlSuffix) {
		...
		String urlString = HTTPUtil.getServerUrl(baseServerUrl, urlSuffix);
		...
		try {
			...
			urlConnection = HTTPUtil.openHttpURLConnection(urlString, null);
	...
	public static void logURLConnection(HttpURLConnection urlConnection) throws IOException {
		IOUtil.debug("+logURLConnection(" + urlConnection + ")");
		
		/* extract request parameters, if available. */
		if(urlConnection != null) {
			IOUtil.debug("Request Method:" + urlConnection.getRequestMethod());
			...
			for(String key : requestHeader.keySet()) {
				...
				IOUtil.debug("Key:" + key + ", Value:" + listValue);
			}
			
		}
		
		IOUtil.debug("-logURLConnection()");
	}
		...
		if(IOUtil.isNotNull(servletRequest)) {
			try {
				...
				while(headerNames.hasMoreElements()) {
					...
					IOUtil.debug("headerName:" + headerName + ", headerValue:" + headerValue);
	...
	public static void logServletRequest(HttpServletRequest servletRequest) throws IOException {
		IOUtil.debug("+logServletRequest(" + servletRequest + ")");
		/* extract request parameters, if available. */
	...
	public static HttpURLConnection openHttpURLConnection(URL url, Proxy proxy) throws IOException {
		return (IOUtil.isNotNull(url) ? (HttpURLConnection) (IOUtil.isNotNull(proxy) ? url.openConnection(proxy) : url.openConnection()) : null);
	}
	...
	public static HttpURLConnection openHttpURLConnection(URL url) throws IOException {
		return (IOUtil.isNotNull(url) ? (HttpURLConnection) url.openConnection() : null);
	}
	...
	public static HttpsURLConnection openHttpsURLConnection(URL url, Proxy proxy) throws IOException {
		return (IOUtil.isNotNull(url) && url.getProtocol().equals("https") ? (HttpsURLConnection) (IOUtil.isNotNull(proxy) ? url.openConnection(proxy) : url.openConnection()) : null);
	}
	...
	public static void close(HttpURLConnection urlConnection) {
		if(IOUtil.isNotNull(urlConnection)) {
			try {
	...
	public static void setConnectionDefaultProperties(final HttpURLConnection urlConnection, final String requestMethod) throws IOException {
		if(IOUtil.isNotNull(urlConnection)) {
			 ...
			 * input. It cannot be set after the connection is established.
			 */
			urlConnection.setDoInput(true);
			
			// request method (i.e GET/POST/PUT etc)
			if(IOUtil.isNotNullOrEmpty(requestMethod)) {
	...
	public static String toCookyString(Map<String, String> mapCookies) {
		...
		if(!IOUtil.isNullOrEmpty(mapCookies)) {
			...
			if(!IOUtil.isNullOrEmpty(cookyBuilder)) {
			...
			for(String key : mapCookies.keySet()) {
				...
				if(IOUtil.isNotNullOrEmpty(value)) {
					if(equals(Headers.COOKIE, key)) {
						...
						if(!IOUtil.isNullOrEmpty(oldCookies)) {
	...
	public static void setRequestCookies(HttpURLConnection urlConnection, String cookies) {
		if(IOUtil.isNotNull(urlConnection) && IOUtil.isNotNullOrEmpty(cookies)) {
			IOUtil.debug(Headers.COOKIE + ":" + cookies);
	...
	public static String toUrlQueryString(Map<String, Object> requestParameters) {
		...
		if(!IOUtil.isNullOrEmpty(requestParameters)) {
			...
			for(String key : requestParameters.keySet()) {
				...
				value = SecurityHelper.encodeWithURLEncoder(value, IOUtil.UTF_8);
	...
	public static void setQueryString(HttpURLConnection urlConnection, final String queryString) throws IOException {
		...
		if(IOUtil.isNotNull(urlConnection) && IOUtil.isNotNullOrEmpty(queryString)) {
			IOUtil.writeBytes(queryString.getBytes(), urlConnection.getOutputStream(), true);
		}
	}
	...
	public static HttpResponse executeRequest(final String urlString, final String httpMethod, final Map<String, String> requestHeaders, final Map<String, Object> requestParameters, final boolean closeStream) {
		...
		try {
			if(IOUtil.isNullOrEmpty(urlString)) {
			...
			if(Methods.GET.equalsIgnoreCase(httpMethod)) {
				...
				if(IOUtil.isNotNullOrEmpty(queryString)) {
					requestBuilder.append("?").append(queryString);
				}
				urlConnection = HTTPUtil.openHttpURLConnection(requestBuilder.toString());
			} else {
				urlConnection = HTTPUtil.openHttpURLConnection(urlString);
			}
			...
			if(operationResponse.getResponseCode() == 200) {
				byte[] dataBytes = IOUtil.readBytes(urlConnection.getInputStream(), closeStream);
	...
	/**
	 * Executes the GET request of the specified
	 * <code>urlString</code> with <code>requestHeaders</code> and
	 * <code>requestParameters</code> using <code>HttpURLConnection</code>. The
	 * <code>OperationResponse</code> object is returned as response which
	 * contains the data/error, if any, including response headers information.
	...
	/**
	 * Executes the POST request of the specified
	 * <code>urlString</code> with <code>requestHeaders</code> and
	 * <code>requestParameters</code> using <code>HttpURLConnection</code>. The
	 * <code>OperationResponse</code> object is returned as response which
	 * contains the data/error, if any, including response headers information.
	...
	public static Map<String, List<String>> getHeaders(HttpResponse operationResponse) {
		return (IOUtil.isNull(operationResponse) ? null : operationResponse.getResponseHeaders());
	}
	...
	public static Map<String, String> extractCookies(String stringCookies) {
		...
		if(IOUtil.isNotNullOrEmpty(stringCookies)) {
	...
	public static Map<String, String> extractCookies(Map<String, List<String>> responseHeaders) {
		...
		if(!IOUtil.isNullOrEmpty(responseHeaders)) {
			List<String> allCookies = responseHeaders.get(Headers.SET_COOKIE);
			System.out.println("allCookies:" + allCookies);
			if(!IOUtil.isNullOrEmpty(allCookies)) {
				mapCookies = new HashMap<String, String>();
				for(String stringCookie : allCookies) {
					Map<String, String> extractedCookies = HTTPUtil.extractCookies(stringCookie);
					if(!IOUtil.isNullOrEmpty(extractedCookies)) {
	...
	public static String getHeader(HttpResponse operationResponse, String key) {
		return getHeader(HTTPUtil.getHeaders(operationResponse), key);
	}
	...
	public static Object getValueForKey(Map<String, Object> mapKeyValues, String key, Object defaultValue) {
		...
		// ", " + defaultValue + ")");
		Object value = null;
		if(!IOUtil.isNullOrEmpty(mapKeyValues) && !IOUtil.isNullOrEmpty(key)) {
			value = mapKeyValues.get(key);
		}
		
		// return default value if the value is null or empty.
		if(IOUtil.isNull(value)) {
			value = defaultValue;
		}
	...
	public static String getContentDispositionFileNameValue(String contentDisposition) {
		...
		if(!IOUtil.isNullOrEmpty(contentDisposition)) {
	...
	public static List<String> getExcludedHeaders() {
		if(IOUtil.isNull(excludedHeaders)) {
	...
	public static String[] getHeadersIgnored() {
		if(IOUtil.isNull(headersIgnored)) {
	...
	public static List<String> getExcludedParameters() {
		if(IOUtil.isNull(excludedParameters)) {
	...
	public static boolean isExcludedParameter(String paramName) {
		...
		if(IOUtil.isNotNullOrEmpty(paramName) && !IOUtil.isNullOrEmpty(getExcludedParameters())) {
	...
	public static void removeExcludedParameters(SortedMap<String, Object> sortedParameters) {
		if(!IOUtil.isNullOrEmpty(sortedParameters) && !IOUtil.isNullOrEmpty(getExcludedParameters())) {
			for(int i = 0; i < excludedParameters.size(); i++) {
	...
	public static String paramValuesAsHashString(SortedMap<String, Object> requestParameters) {
		...
		if(!IOUtil.isNullOrEmpty(requestParameters)) {
			...
			String paramValuesAsString = HTTPUtil.paramValuesAsString(sortedParameters);
	...
	public static boolean isExcludedMethodRequest(String requestMethodName, String... excludedMethods) {
		...
		if(!IOUtil.isNullOrEmpty(requestMethodName) && !IOUtil.isNullOrEmpty(excludedMethods)) {
	...
	public static List<String> getExcludedMethods() {
		if(IOUtil.isNull(excludedMethods)) {
	...
	public static void filterRequestParameters(SortedMap<String, Object> requestParameters, boolean excludedMethodRequest) {
		if(excludedMethodRequest && !IOUtil.isNullOrEmpty(requestParameters)) {
	...
	public static String paramValuesAsString(String... paramValues) {
		...
		if(!IOUtil.isNullOrEmpty(paramValues)) {
			StringBuilder hashBuffer = new StringBuilder();
			Arrays.sort(paramValues);
			for(int i = 0; i < paramValues.length; i++) {
				String paramValue = IOUtil.isNullOrEmpty(paramValues[i]) ? "" : paramValues[i];
	...
	public static String paramValuesAsString(List<String> paramValues) {
		...
		if(!IOUtil.isNullOrEmpty(paramValues)) {
			...
			for(String paramValue : paramValues) {
				paramValue = IOUtil.isNullOrEmpty(paramValue) ? "" : paramValue;
				hashBuffer.append(paramValue);
			}
	...
	public static String paramValuesAsString(SortedMap<String, ? extends Object> requestParameters) {
		...
		if(!IOUtil.isNullOrEmpty(requestParameters)) {
			...
			for(String key : requestParameters.keySet()) {
				...
				String valueAsString = IOUtil.isNull(value) ? "" : value.toString();
	...
	public static void addToHashBuffer(final StringBuilder hashBuffer, String... values) {
		if(IOUtil.isNotNull(hashBuffer) && !IOUtil.isNullOrEmpty(values)) {
			for(int i = 0; i < values.length; i++) {
				if(!IOUtil.isNullOrEmpty(values[i])) {
		...
		if(!IOUtil.isNullOrEmpty(urlString)) {
			...
			if(startIndex >= 0) {
				...
				if(endIndex > -1 && endIndex < startIndex) {
					int slashIndex = urlString.lastIndexOf(IOUtil.SLASH);
					if(slashIndex != -1) {
	...
	public static boolean isDigits(String string) {
		return (IOUtil.isNotNullOrEmpty(string) && string.matches("^[0-9]*$"));
	}
	...
	public static boolean startsWith(String value, String... prefixes) {
		boolean result = false;
		if(!IOUtil.isNullOrEmpty(value) && prefixes != null) {
	...
	public static class Pairs<K, V> implements Serializable {
		...
		public static Pairs<String, String> newPair(String keyValueString) {
			...
			if(IOUtil.isNotNullOrEmpty(keyValueString)) {
	 ...
	 * certificate in addition to those that are in the system TrustStore.
	 * Also handles an out-of-order certificate chain, as is often produced by
	 * Apache's mod_ssl
	 *
	 * @author Rohtash Singh Lakra
	...
	public static final class CustomTrustManager implements X509TrustManager {
		...
		/**
		 * @param trustStore A KeyStore containing the server certificate that
		 *            should be trusted
		 ...
		 * validated and is trusted
		 * for client SSL authentication based on the authentication type. The
		 * authentication type is
		 * determined by the actual certificate used. For instance, if
		 * RSAPublicKey
		 * is used, the authType should be "RSA".
		 * Checking is case-sensitive.
		 * Defers to the default trust manager first, checks the cert supplied
		 * in the ctor if that fails.
		 * 
		 ...
		 * Finds the root certificate in a possibly out-of-order certificate
		 * chain.
		 * 
		...
		/**
		 * A helper method for certificate re-ordering.
		 * Finds the first certificate in the list of certificates that is
		 * signed by
		 * the sigingCert.
		 */
		...
		/**
		 * A helper method for certificate re-ordering.
		 * Finds the certificate in the list of certificates that signed the
		 * signedCert.
		 */
	...
	public static final class SSLFactory {
		...
		public KeyStore loadPKCS12KeyStore(InputStream p12CertInputStream, char[] p12CertPass, boolean closeStream) throws GeneralSecurityException, IOException {
			...
			if(closeStream) {
				IOUtil.safeClose(p12CertInputStream);
			}
		...
		public KeyStore loadPKCS12KeyStore(String p12CertFileName, String p12CertPassword) throws GeneralSecurityException, IOException {
			return loadPKCS12KeyStore(IOUtil.toInputStream(IOUtil.readBytes(p12CertFileName)), p12CertPassword.toCharArray(), false);
		}
		 ...
		 * (typically, the server's CA certificate)
		 * 
		 * @param certificateString A String containing the PEM-encoded
		 *            certificate
	...
	public static final class HttpResponse implements Cloneable {
		...
		public void setResponseHeaders(final Map<String, List<String>> responseHeaders) {
			...
			if(!IOUtil.isNullOrEmpty(responseHeaders)) {
				jsonResponseHeaders = HTTPUtil.toJson(responseHeaders);
			}
		}
		...
		public void setJsonResponseHeaders(byte[] jsonResponseHeaders) {
			if(!IOUtil.isNullOrEmpty(jsonResponseHeaders)) {
				String jsonResponseHeader = IOUtil.toUTF8String(jsonResponseHeaders);
				this.responseHeaders = HTTPUtil.jsonHeadersAsMap(jsonResponseHeader);
			}
		}
		...
		public void setDataBytes(byte[] dataBytes) {
			...
			System.out.println("dataBytes:" + IOUtil.toUTF8String(dataBytes));
		}
		...
		public String toString() {
			...
			sBuilder.append("dataBytes:").append(IOUtil.toUTF8String(getDataBytes())).append("\n");
			
			if(this.error != null) {
				sBuilder.append("\n\n").append(HTTPUtil.toString(getError())).append("\n\n");
			}
